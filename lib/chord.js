...
function Chord(arg1,name){
  ...
  if(Array.isArray(arg1)){
    ...
    //if no name was specify, try to find one
    if(name == undefined)
      this.findName()
...
Chord.prototype.findName = function(){
 let permutations = findPermutations(this.getNotes())
 ...
 for(let i = 0;i<length;i++){
   ...
   for(let j = 0;j<permutations[1][i].length;j++){
     x.push(permutations[0][i].plusInterval(permutations[1][i][j]))
   }
   x = x.map(n =>n.getName())
   console.log(x + ' - '+permutations[1][i].map(interval =>interval.getName()) + ' - Name: ' + name) */
...
const findNameFromPermutation = function(root,intervals,slash){
  ...
  if (arrayRemoveCheck(intervals, 'm7')) {
    ...
    if (arrayRemoveCheck(intervals, 'M2')) {
        number = '9';
    }
    if (arrayRemoveCheck(intervals, 'P4')) {
        number = '11';
    }
    if (arrayRemoveCheck(intervals, 'A4')) {
        number = '13';
    }
  ...
  if(slash != null)
    mods+= '/' + slash.getRootName()
  
  return name+number+mods
