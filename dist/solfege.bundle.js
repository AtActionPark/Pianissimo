...
var findNoteNameFromInterval = function findNoteNameFromInterval(note, interval) {
  //Add the interval number to the root note to find the result note
  //We dont care about alterations, just about the note index in the wholeNotesOrder list
  var rootNote = note.getRootAsLetter();
...
var displayNameAsText = function displayNameAsText(interval) {
  var order = interval.getOrderAsString();
  if (interval.getName() == "P1" || interval.getName() == "d2") order = '';
  if (interval.getNumberText() == undefined) return '?';
...
function Chord(arg1, name) {
  ...
  if (!Helpers.isNote(arg1)) arg1 = new Note(arg1);
...
Chord.prototype.transpose = function (interval, order) {
  ...
  if (slash) symbols += '/' + notes[0].getRoot();
...
//Private
//Setters
var setTonic = function setTonic(chord, newTonic) {
  chord[_tonic] = newTonic;
};
var setSymbols = function setSymbols(chord, newSymbols) {
  chord[_symbols] = newSymbols;
};
var setName = function setName(chord, newName) {
  chord[_name] = newName;chord.chord = newName;
};
...
var parseName = function parseName(name) {
  ...
  if (name.charAt(1) == '5') powerchord = true;
...
var buildChord = function buildChord(chord, parsed) {
  ...
  if (parsed['otherNum'] != null) {
    for (var i = 0; i < parsed['otherNum'].length; i++) {
      ...
      if (num == 6) {
        ...
        } else intervals = addToIntervals(intervals, 'M' + num);
      }
    }
  }
  ...
  if (parsed['sharps'] != null) {
    for (var _i2 = 0; _i2 < parsed['sharps'].length; _i2++) {
      ...
      if (s == 7) intervals = addToIntervals(intervals, 'M7');else {
  ...
  if (parsed['flats'] != null) {
    for (var _i3 = 0; _i3 < parsed['flats'].length; _i3++) {
      ...
      if (f % 7 == 1 || f % 7 == 4 || f % 7 == 5) intervals = addToIntervals(intervals, 'd' + f);else intervals = addToIntervals(intervals, 'm' + f);
    }
  }
  if (parsed['add'] != null) {
    for (var _i4 = 0; _i4 < parsed['add'].length; _i4++) {
      ...
      if (altb) _quality = _s % 7 == 1 || _s % 7 == 4 ? 'd' : 'm';else if (alts) _quality = _s % 7 == 0 ? 'M' : 'A';
  ...
  if (parsed['dim'] != null) {
    ...
    if (_nb == 3) intervals = addToIntervals(intervals, 'd3');
    if (_nb >= 7) intervals = addToIntervals(intervals, 'd7');
  }
  if (parsed['sus'] != null) {
    for (var _i5 = 0; _i5 < parsed['sus'].length; _i5++) {
      ...
      if (_s2 == 2 || _s2 == 9) intervals = addToIntervals(intervals, 'M2');else if (_s2 == 4) intervals = addToIntervals(intervals, 'P4');else if (_s2 == 24) {
        intervals = addToIntervals(intervals, 'M2');
        intervals = addToIntervals(intervals, 'P4');
      } else if (_s2 == null) intervals = addToIntervals(intervals, 'P4');
    }
  }
  if (parsed['dom'] != null) {
    intervals = addToIntervals(intervals, 'M3');
...
var hasNote = function hasNote(chord, note) {
  ...
  for (var i = 0; i < notes.length; i++) {
    if (notes[i].replace(/\d+/, '') == note) return i;
  }
  return -1;
};
...
var findNameFromPermutation = function findNameFromPermutation(root, intervals, slash) {
  ...
  if (arrayRemoveCheck(intervals, 'M3')) {
    ...
    mods += 'sus2';
    if (arrayRemoveCheck(intervals, 'P4')) mods += '4';
    if (!arrayRemoveCheck(intervals, 'P5')) {
      mods += 'no5';
    }
    if (arrayRemoveCheck(intervals, 'M6')) {
    ...
    if (arrayRemoveCheck(intervals, 'P8')) powerChord = true;else {
      no3 = true;
      if (arrayRemoveCheck(intervals, 'M6')) {
        number += '6';
      }
  ...
  for (var i = 1; i < intervals.length; i++) {
    var x = intervals[i];
    if (x == 'A9') mods += '♯9';else if (x == 'm9') mods += '♭9';else if (x == 'A11') mods += '♯11';else if (x == 'd11') mods += '♭11';else if (x == 'A13') mods += '♯13';else if (x == 'm13') mods += '♭13';else {
      var n = x.match(/\d+/g);
      var q = x.slice(0, 1);
      if (n % 7 == 1 || n % 7 == 4 || n % 7 == 5) {
        if (q == 'P') mods += 'add' + n;else if (q == 'A') mods += 'add♯' + n;else if (q == 'd') mods += 'add♭' + n;
      } else {
        if (q == 'M') mods += 'add' + n;else if (q == 'm') mods += 'add♭' + n;else if (q == 'A') mods += 'add♯' + n;else if (q == 'd') mods += 'add♭♭' + n;
      }
    }
  }
  ...
  if (slash != null) mods += '/' + slash.getRoot();
...
var findIntervalsFromNotes = function findIntervalsFromNotes(notes) {
  ...
  for (var i = 0; i < notes.length; i++) {
    ...
    if (inter.getOrder() == 'descending') inter = inter.invert();
    intervals.push(inter);
  }
...
var findPossibleNames = function findPossibleNames(chord) {
  ...
  for (var i = 0; i < length; i++) {
    ...
    for (var j = 0; j < permutations[1][i].length; j++) {
      ...
      if (p != 'impossible to compute') x.push(p);
      if (permutations[2][i] != null) slash = permutations[2][i];
    }
    if (slash != null) x.unshift(slash);
    x = x.map(function (n) {
      return n.getName();
    });
...
//Private
//Setters
var setType = function setType(scale, newType) {
    scale[_type] = newType;
};
var setDegree = function setDegree(scale, newDegree) {
    scale[_degree] = newDegree;
};
